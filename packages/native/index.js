...
if (!nativeBinding || forceWasi) {
  ...
  if (!wasiBindingLoaded && (!__napiWasiFlavorRequested || __napiWasiFlavor === 'wasm32-wasi')) {
    ...
    try {
      candidateError = __napiWasiResolveCandidate('./gpuix-native.wasi.cjs', false, ['./gpuix-native.wasm32-wasi.debug.wasm', './gpuix-native.wasm32-wasi.wasm'])
  ...
  if (!wasiBindingLoaded && (!__napiWasiFlavorRequested || __napiWasiFlavor === 'wasm32-wasi')) {
    ...
    let candidateFailed = false
    try {
      candidateError = __napiWasiResolveCandidate('@gpuix/native-wasm32-wasi', true, undefined)
      candidateFailed = candidateError !== null
      if (!candidateFailed) {
        if (process.env.NAPI_RS_ENFORCE_VERSION_CHECK && process.env.NAPI_RS_ENFORCE_VERSION_CHECK !== '0') {
          const bindingPackageVersion = require('@gpuix/native-wasm32-wasi/package.json').version
...
const { AvailableUpdate, GpuixRenderer, TestGpuixRenderer, checkUpdate, hasTestGpuixRenderer } = nativeBinding
export { AvailableUpdate }
export { GpuixRenderer }
export { TestGpuixRenderer }
export { checkUpdate }
