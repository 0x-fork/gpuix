...
State is seeded from the renderer with a lazy initializer, and the hook only rerenders when the width or height actually changes, so a drag-resize cannot flood React with renders. This mirrors what `useWindowInsets()` already does; the two hooks now share the same pull-based shape and the same `intervalMs` option.
